...
class Web3Passkey implements IWeb3Passkey {
  ...
  Mnemonic generateMnemonic(
      {int length = 12, Language language = Language.english}) {
    if ([12, 15, 18, 21, 24].contains(length) == false) {
      throw ArgumentError(
          'The words length must be 12, 15, 18, 21 or 24', length as String?);
    }
  ...
  EthPassKey createEthPassKey(IMnemonic mnemonic, String passPhrase) {
    ...
    final Uint8List seed =
        Pbkdf2.createSeedFromMnemonicAndPassword(mnemonic, passPhrase);
  ...
  @override
  EthBip32PassKey createDerivateEthPassKey(
      IMnemonic mnemonic, String passPhrase) {
    // Semilla
    final Uint8List seed =
        Pbkdf2.createSeedFromMnemonicAndPassword(mnemonic, passPhrase);
  ...
  Future<EthPassKey> saveEthPasskey(
      String documentId, String name, EthPassKey passKey) async {
    ...
    final Encrypted encPrivateKey =
        algorithm.encrypt(passKey.keyPair.privateKey.toString(), iv: iv);
    final Encrypted encPublicKey =
        algorithm.encrypt(passKey.keyPair.publicKey.toString(), iv: iv);
    ...
    final decName =
        algorithm.decrypt(Encrypted.fromBase64(result.name), iv: iv);
    final decPrivateKey =
        algorithm.decrypt(Encrypted.fromBase64(result.privateKey), iv: iv);
    final decPublicKey =
        algorithm.decrypt(Encrypted.fromBase64(result.publicKey), iv: iv);

    // Encapsular claves publica y privada
    final ECKeyPair keyPair = ECKeyPair.fromKeyPairInt(
        BigInt.parse(decPrivateKey), BigInt.parse(decPublicKey));
  ...
  Future<EthPassKey> getCurrentEthPassKey() async {
    ...
    // Crear derivador de llaves de accesos usando las claves
    return EthPassKey.fromKeyPairExtender(
        keyPair, result.passkeyID, decName, null);
  }
  ...
  Future<List<EthPassKey>> getAllEthPasskey() async {
    ...
    final List<EthPassKey> list = result.map((element) {
      final decName =
          algorithm.decrypt(Encrypted.fromBase64(element.name), iv: iv);
      final decPrivateKey =
          algorithm.decrypt(Encrypted.fromBase64(element.privateKey), iv: iv);
      final decPublicKey =
          algorithm.decrypt(Encrypted.fromBase64(element.publicKey), iv: iv);
      ...
      // Crear derivador de llaves de accesos usando las claves
      return EthPassKey.fromKeyPairExtender(
          keyPair, element.passkeyID, decName, null);
    }).toList();
